...
const (
	...
	mdExt4Bytes uint64 = humanize.MiByte
)
...
func adjustScmSize(availBytes uint64) uint64 {
	...
	const mdExt4Bytes uint64 = humanize.MiByte
	const mdBytes uint64 = mdCapSize + mdExt4Bytes
...
func TestServer_adjustScmSize(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"single mountPoint": {
			...
			output: ExpectedOutput{
				availableBytes: []uint64{uint64(64)*humanize.GiByte - mdcapSize - mdExt4Bytes},
			},
		},
		"three mountPoints": {
			input: DataInput{
			...
			output: ExpectedOutput{
				availableBytes: []uint64{
					uint64(64)*humanize.GiByte - 1024*humanize.MiByte - mdExt4Bytes,
					uint64(32)*humanize.GiByte - 1024*humanize.MiByte - mdExt4Bytes,
					uint64(128)*humanize.GiByte - 1024*humanize.MiByte - mdExt4Bytes,
				},
			},
		},
		"Missing SCM": {
